...
const fonts = `@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;500;600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap');`;
...
export default function AdminDashboard() {
  ...
  // Separate bucket for church-approval queue (populated on load), independent
  // of the `churches` state that's driven by the Churches tab's search debounce.
  ...
  const fetchData = useCallback(async () => {
    setLoadingData(true);
    const [usersRes, churchCountRes, pendingChurchesRes, reviewsRes, flagsRes, reportsRes] = await Promise.all([
      supabase.from("profiles").select("*").order("created_at", { ascending: false }),
      supabase.from("churches").select("*", { count: "estimated", head: true }),
      // Pending & rejected churches for the approval queue. Include submitter info.
      supabase
        .from("churches")
        .select("*, profiles!churches_added_by_fkey(display_name, id)")
    ...
    setPendingChurches(pendingChurchesRes.data || []);
    // Leave `churches` empty here — it's populated by the Churches-tab search.
    setChurches([]);
  ...
  /* --- CHURCH APPROVAL (separate from review approval) --- */
  const updateChurchStatus = async (churchId, status) => {
    const updateData = { status };
    ...
    if (!error) {
      // Remove from the pending queue once it's decided; the Churches-tab
      // search can re-surface it if the admin needs to edit it later.
      setPendingChurches(prev => prev.filter(c => c.id !== churchId));
      showToast(
        status === "approved"
          ? "Church approved & published"
          : status === "rejected"
          ? "Church rejected"
          : "Church updated",
        status === "rejected" ? T.amber : T.green,
      );
    } else {
      console.error("Failed to update church:", error);
      showToast(`Failed to update church: ${error.message || "Unknown error"}`, T.red);
    }
  };

  const rejectChurchWithConfirm = (churchId, churchName) => {
    setConfirmDialog({
      title: "Reject Church",
      message: `Reject "${churchName}"? It will be hidden from the public site. The submitter can resubmit a corrected version.`,
      confirmLabel: "Reject",
      variant: "amber",
      onConfirm: () => updateChurchStatus(churchId, "rejected"),
    });
  };
        ...
        {!loadingData && tab === "overview" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
              ...
              <Stat label="Pending Reviews" value={pendingReviews} sub="Awaiting approval" color={T.accent} icon="⏳" />
              <Stat label="Pending Churches" value={pendingChurches.filter(c => c.status === "pending").length} sub="Awaiting approval" color={T.accent} icon="⛪" />
              <Stat label="Flagged" value={flaggedReviews} sub="Needs attention" color={T.amber} icon="⚠️" />
            </div>

            {/* Pending Churches Queue */}
            {pendingChurches.filter(c => c.status === "pending").length > 0 && (
              <div style={{ padding: "20px", borderRadius: T.radius, background: T.surface, border: `1px solid ${T.border}`, marginBottom: 20 }}>
                <div style={{ fontSize: 15, fontWeight: 700, fontFamily: T.heading, marginBottom: 14, letterSpacing: "-0.02em" }}>
                  Pending Churches ({pendingChurches.filter(c => c.status === "pending").length})
                </div>
                <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 14 }}>
                  New churches submitted by users. Approve to make public. Reject hides them from the site.
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {pendingChurches.filter(c => c.status === "pending").slice(0, 10).map(ch => (
                    <div key={ch.id} style={{ padding: "14px", borderRadius: T.radiusSm, background: T.accentSoft, border: `1px solid ${T.accentBorder}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: T.accent }}>{ch.name}</div>
                        <Badge status="pending" />
                      </div>
                      <div style={{ fontSize: 12, color: T.textSoft, lineHeight: 1.5, marginBottom: 4 }}>
                        {[ch.address, ch.city, ch.state, ch.zip].filter(Boolean).join(", ")}
                      </div>
                      <div style={{ fontSize: 11, color: T.textMuted, marginBottom: 8 }}>
                        {ch.denomination} · Submitted by {ch.profiles?.display_name || "unknown"}
                        {ch.website ? <> · <a href={ch.website.startsWith("http") ? ch.website : `https://${ch.website}`} target="_blank" rel="noopener noreferrer" style={{ color: T.accent }}>{ch.website}</a></> : null}
                      </div>
                      <div style={{ display: "flex", gap: 6 }}>
                        <Button onClick={() => updateChurchStatus(ch.id, "approved")} variant="green">Approve</Button>
                        <Button onClick={() => rejectChurchWithConfirm(ch.id, ch.name)} variant="amber">Reject</Button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(420px, 1fr))", gap: 12 }}>
              {filteredReviews.map(r => {
                ...
                const isSelected = selectedReviews.has(r.id);
                ...
                const toggleExpand = () => {
                  ...
                  isExpanded ? newSet.delete(r.id) : newSet.add(r.id);
                ...
                const toggleSelect = (e) => {
                  ...
                  isSelected ? newSet.delete(r.id) : newSet.add(r.id);
                  setSelectedReviews(newSet);
                };

                return (
                  <div key={r.id} style={{
                    borderRadius: 12, background: isSelected ? `${T.accent}11` : T.surface, border: `1px solid ${isSelected ? T.accent : T.border}`,
                    borderLeft: `3px solid ${borderLeft}`,
                    transition: "all 0.15s ease",
                    display: "flex", flexDirection: "column",
                  }}>
                    <div style={{ padding: "16px 18px 12px", cursor: "pointer" }} onClick={toggleExpand}>
                      {/* Top row: checkbox + avatar + name + status */}
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <input type="checkbox" checked={isSelected} onChange={toggleSelect} onClick={(e) => e.stopPropagation()} style={{ cursor: "pointer", width: 16, height: 16, accentColor: T.accent }} />
                          <div style={{ width: 32, height: 32, borderRadius: 16, background: `hsl(${((r.profiles?.display_name || "A").charCodeAt(0) * 47) % 360}, 40%, 25%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", fontFamily: T.heading, flexShrink: 0 }}>{(r.profiles?.display_name || "A").charAt(0)}</div>
                    ...
                    {isExpanded && (
                      <div style={{ borderTop: `1px solid ${T.border}`, padding: "14px 18px" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, alignItems: "center", justifyContent: "space-between" }}>
                          ...
                          <button onClick={(e) => { e.stopPropagation(); const s = new Set(expandedReviews); s.delete(r.id); setExpandedReviews(s); }} style={{ fontSize: 11, color: T.textMuted, background: "none", border: "none", cursor: "pointer", padding: "4px 8px" }}>Collapse</button>
                        </div>
                      </div>
                    )}
                  </div>
