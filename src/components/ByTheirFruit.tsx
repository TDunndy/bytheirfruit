                  ...
                  <div onClick={() => viewChurch(church)} style={{ padding: "22px", borderRadius: T.radius, cursor: "pointer", background: T.surface, border: `1.5px solid ${T.border}`, transition: "all 0.2s", position: "relative" }}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 13, color: T.textMuted }}>{church.denomination} Â· {church.city}, {church.state}{church.size ? ` Â· ${church.size}` : ""}{nearMeDistances[church.id] != null && <span style={{ color: T.accent, fontWeight: 600 }}> Â· {nearMeDistances[church.id] < 1 ? "< 1" : Math.round(nearMeDistances[church.id])} mi</span>}</div>
